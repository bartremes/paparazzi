...
static inline void bluegiga_read_buffer( struct pprz_transport *t ) {
  bluegiga_receive();               // read data from spi if any available
  do {
    int c = 0;
    do
    {
      parse_pprz( t, bluegiga_p.rx_buf[(bluegiga_p.rx_extract_idx + c++) % BLUEGIGA_BUFFER_SIZE]);
    } while ((bluegiga_p.rx_extract_idx + c < bluegiga_p.rx_insert_idx ) && !(t->trans_rx.msg_received) );
    ...
    if (t->trans_rx.msg_received)
      {
        ...
        bluegiga_p.rx_extract_idx = (bluegiga_p.rx_extract_idx + c) % BLUEGIGA_BUFFER_SIZE;
      }
